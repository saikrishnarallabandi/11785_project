...
A = np.load('dev_input.npy')
B = np.load('dev_output.npy')
...
def train_model():
  ...
  for epoch in range(10):
    ...
    for ctr, (utterance, phoneme) in enumerate(train_data):
          ...
          inf = float("inf")
          nan = float("nan")
          if loss_sum == inf or loss_sum == -inf or loss_sum == nan :
          ...
          if ctr % 20 == 1:
               g = open('logfile','a')
               g.write("  Training Loss after " + str(ctr) +  " sequences is : " + str(float(total_loss/(ctr+1))) + '\n') 
               g.close()
